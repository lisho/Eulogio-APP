...
interface SidebarProps {
  ...
  onDeleteConversation: (conversationId: string) => void; // New prop
}
          ...
          {previousConversations.sort((a, b) => b.timestamp - a.timestamp).map((conv) => (
            <div key={conv.id} className="flex items-center justify-between group">
              <button
                ...
                title={conv.name}
                className={`flex-grow text-left px-3 py-2 rounded-md text-sm font-medium transition-colors duration-150 truncate ${
                  conv.id === currentConversationId
                    ? 'bg-rose-100 text-primary-dark'
                    : 'text-gray-700 hover:bg-rose-50 hover:text-primary-dark'
                }`}
              >
                {conv.name || new Date(conv.timestamp).toLocaleString()}
              </button>
              <button
                onClick={(e) => {
                ...
                className="p-1.5 text-gray-400 hover:text-red-600 rounded-full hover:bg-red-100 opacity-0 group-hover:opacity-100 focus:opacity-100 transition-opacity duration-150 ml-1 flex-shrink-0"
                aria-label={`Borrar conversación: ${conv.name}`}
                title={`Borrar conversación: ${conv.name}`}
              >
